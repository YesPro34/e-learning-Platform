...
//           const response = await fetch(`${YOUTUBE_PLAYLIST_ITEMS_API}?part=snippet&maxResults=12&playlistId=${courseID.id}&key=AIzaSyBuPaBnOxJcNNf6GgsHX-ps-B_XwUt-LtQ`);
...
import Title from '@/app/(DashboardLayout)/components/titles/Title';
import VideoCard from '@/app/(DashboardLayout)/components/videocard/VideoCard';
import { getCourses } from '@/app/(DashboardLayout)/helpers/getCourses';
import BaseCard from '@/app/(DashboardLayout)/components/shared/BaseCard';
      ...
      try {
        while (true) {
          const response = await fetch(
            `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&pageToken=${nextPageToken}&playlistId=${courseID.id}&key=AIzaSyBuPaBnOxJcNNf6GgsHX-ps-B_XwUt-LtQ`
          );
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
